...
type Renderer interface {
	...
	RenderTemplateParamsWithStrategy(tmpl *argoappsv1.Application, syncPolicy *argoappsv1.ApplicationSetSyncPolicy, params map[string]interface{}, strategy argoappsv1.TemplateStrategy) (*argoappsv1.Application, error)
	Replace(tmpl string, replaceMap map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (string, error)
	ReplaceWithStrategy(tmpl string, replaceMap map[string]interface{}, strategy argoappsv1.TemplateStrategy) (string, error)
}
...
func (r *Render) deeplyReplace(copy, original reflect.Value, replaceMap map[string]interface{}, strategy argoappsv1.TemplateStrategy) error {
	...
	case reflect.Ptr:
		...
		if err := r.deeplyReplace(copy.Elem(), originalValue, replaceMap, strategy); err != nil {
	...
	case reflect.Interface:
		...
		if originalValue.IsValid() {
			...
			if err := r.deeplyReplace(copyValue, originalValue, replaceMap, strategy); err != nil {
				// Not wrapping the error, since this is a recursive function. Avoids excessively long error messages.
				return err
			}
			copy.Set(copyValue)
	...
	case reflect.Struct:
		for i := 0; i < original.NumField(); i += 1 {
			...
			if currentType == "time.Time" {
				...
				err = r.deeplyReplace(jsonCopy, jsonOriginal, replaceMap, strategy)
				if err != nil {
			...
			} else if err := r.deeplyReplace(copy.Field(i), original.Field(i), replaceMap, strategy); err != nil {
				// Not wrapping the error, since this is a recursive function. Avoids excessively long error messages.
				return err
			}
		}
	...
	case reflect.Slice:
		...
		for i := 0; i < original.Len(); i += 1 {
			if err := r.deeplyReplace(copy.Index(i), original.Index(i), replaceMap, strategy); err != nil {
				// Not wrapping the error, since this is a recursive function. Avoids excessively long error messages.
				return err
			}
		}
	...
	case reflect.Map:
		...
		for _, key := range original.MapKeys() {
			...
			if key.Kind() == reflect.String {
				strategy := argoappsv1.TemplateStrategy{}
				templatedKey, err := r.ReplaceWithStrategy(key.String(), replaceMap, strategy)
				if err != nil {
					// Not wrapping the error, since this is a recursive function. Avoids excessively long error messages.
					return err
				}
	...
	case reflect.String:
		...
		templated, err := r.ReplaceWithStrategy(strToTemplate, replaceMap, strategy)
		if err != nil {
			// Not wrapping the error, since this is a recursive function. Avoids excessively long error messages.
			return err
		}
...
func (r *Render) RenderTemplateParams(tmpl *argoappsv1.Application, syncPolicy *argoappsv1.ApplicationSetSyncPolicy, params map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (*argoappsv1.Application, error) {
	strategy := argoappsv1.TemplateStrategyFromLegacyOptions(useGoTemplate, goTemplateOptions)
	return r.RenderTemplateParamsWithStrategy(tmpl, syncPolicy, params, strategy)
}

func (r *Render) RenderTemplateParamsWithStrategy(tmpl *argoappsv1.Application, syncPolicy *argoappsv1.ApplicationSetSyncPolicy, params map[string]interface{}, strategy argoappsv1.TemplateStrategy) (*argoappsv1.Application, error) {
	if tmpl == nil {
	...
	if err := r.deeplyReplace(copy, original, params, strategy); err != nil {
		return nil, err
	}
...
func (r *Render) RenderGeneratorParams(gen *argoappsv1.ApplicationSetGenerator, params map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (*argoappsv1.ApplicationSetGenerator, error) {
	strategy := argoappsv1.TemplateStrategyFromLegacyOptions(useGoTemplate, goTemplateOptions)
	return r.RenderGeneratorParamsWithStrategy(gen, params, strategy)
}

func (r *Render) RenderGeneratorParamsWithStrategy(gen *argoappsv1.ApplicationSetGenerator, params map[string]interface{}, strategy argoappsv1.TemplateStrategy) (*argoappsv1.ApplicationSetGenerator, error) {
	if gen == nil {
...
func (r *Render) Replace(tmpl string, replaceMap map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (string, error) {
	strategy := argoappsv1.TemplateStrategyFromLegacyOptions(useGoTemplate, goTemplateOptions)
	return r.ReplaceWithStrategy(tmpl, replaceMap, strategy)
}

// ReplaceWithStrategy executes basic string substitution of a template with replacement values.
// remaining in the substituted template.
func (r *Render) ReplaceWithStrategy(tmpl string, replaceMap map[string]interface{}, strategy argoappsv1.TemplateStrategy) (string, error) {
	switch strategy.Type {
	case argoappsv1.TemplateStrategyTypeGo:
		template, err := template.New("").Funcs(sprigFuncMap).Parse(tmpl)
		if err != nil {
			return "", fmt.Errorf("failed to parse template %s: %w", tmpl, err)
		}
		for _, option := range strategy.Go.TemplateOptions {
	...
	case argoappsv1.TemplateStrategyTypeKCL:
		args, err := kcl.ParseArgs(nil, kcl.WithCode(tmpl))
		if err != nil {
			return "", err
		}
		...
		result, err := kcl.ExecResultToKCLResult(&args, resp, args.GetLogger(), kcl.DefaultHooks)
		if err != nil {
			return "", err
		}
